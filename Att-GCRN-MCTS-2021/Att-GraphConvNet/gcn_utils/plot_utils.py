...
def plot_tsp(p, x_coord, W, W_val, W_target, title="default"):
    ...
    colors = ['r'] + ['c'] * (len(x_coord) - 1)  # Green for 0th node, blue for others
    ...
    nx.draw_networkx_edges(G, pos, edgelist=target_pairs, alpha=1, width=1, edge_color='m')
    p.set_title(title)
    return p

def plot_predictions_cluster(x_nodes_coord, x_edges, x_edges_values, y_edges,
                             y_pred_edges, num_plots=3, jpgname = 'tsp20_0',
                             nodes = 20, mean_rank = 2.0, fnn = 0):
    """
    ...
    """
    #y = F.softmax(y_pred_edges, dim=2)  # B x V x V x voc_edges
    y_probs = y_pred_edges  # Prediction probabilities: B x V x V
    #for f_idx, idx in enumerate(np.random.choice(len(y), num_plots, replace=False)):
    f = plt.figure(0, figsize=(15, 5))
    x_coord = x_nodes_coord
    W = x_edges
    W_val = x_edges_values
    W_target = y_edges
    W_sol_probs = y_probs
    plt1 = f.add_subplot(131)
    plot_tsp(plt1, x_coord, W, W_val, W_target, 'Raw Graph (scale = {} nodes)'.format(nodes))
    plt2 = f.add_subplot(132)
    plot_tsp_heatmap(plt2, x_coord, W_val, W_sol_probs, 'Prediction Heatmap (mean-rank = {})'.format(mean_rank))
    plt3 = f.add_subplot(133)
    plot_tsp(plt3, x_coord, W_target, W_val, W_target, 'Optimized Solution (FNE = {} nodes)'.format(fnn))
    plt.savefig('./figures/heatmap_{}.png'.format(jpgname))
    plt.show()
...
def plot_tsp_heatmap(p, x_coord, W_val, W_pred, title="default"):
    ...
    node_color = ['r'] + ['c'] * (len(x_coord) - 1)  # Green for 0th node, blue for others
...
def plot_predictions(x_nodes_coord, x_edges, x_edges_values, y_edges, y_pred_edges, num_plots=3):
    ...
    for f_idx, idx in enumerate(np.random.choice(len(y), num_plots, replace=False)):
        ...
        plot_tsp(plt1, x_coord, W, W_val, W_target, 'Raw Graph (scale = 100 nodes)')
        plt2 = f.add_subplot(132)
        plot_tsp_heatmap(plt2, x_coord, W_val, W_sol_probs, 'Prediction Heatmap')
        plt3 = f.add_subplot(133)
        plot_tsp(plt3, x_coord, W_target, W_val, W_target, 'Optimized Solution (scale = 100 nodes)')
        plt.savefig('./figures/heatmap_{}.png'.format(num))
        num+=1
        plt.show()
...
def plot_predictions_beamsearch(x_nodes_coord, x_edges, x_edges_values, y_edges, y_pred_edges, bs_nodes, num_plots=3):
    ...
    for f_idx, idx in enumerate(np.random.choice(len(y), num_plots, replace=False)):
        ...
        plt.show()
    plt.savefig('./figures/heatmap_bs.png')
